...
- *[todo]* Supports managing processes like `embedded` `server` executables, embedded `database` executables, or `any other` embedded/ non-embedded executables 
...
* [] Python development server using `py -m http.server` [PLANNED] <!-- If Python version returned above is 3.X. # On Windows, try "python -m http.server" or "py -3 -m http.server". If Python version returned above is 2.X use SimpleHTTPServer. python3 -m http.server OR python -m SimpleHTTPServer -->
