...
if (__dirname.toString().includes("process")) {
    var args = [path.join(__dirname, "../../../../www/files/perl/index.pl")];
} else {
    var args = [path.join(__dirname, "./www/files/perl/index.pl")];
}
...
var proc = obj.process.executeProcess({
    ...
    function (error, stdout, stderr) {
        ...
        if (os.platform() === "win32") {
            assert(!!stdout.toString().includes("This is a headline."), "2. Check STDOUT for perl (pl) server");
            assert(!!stdout.toString().includes("This is body text."), "2. Check STDOUT for perl (pl) server");
        } else {
