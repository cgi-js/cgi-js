...
let proc = obj.process.executeAction("lscommand", "generic", {
    stdio: 'inherit',
    shell: true
}, (error, stdout, stderr) => {
    console.log("Starting Tests for test.cgi.process.execAction dataHandlers tests")
    ...
    console.log("Ending Tests: with above failure test.cgi.process.execAction dataHandlers tests")
    eventEmitter.emit('closeprocess', stdout);
}, (options, proc) => {
    console.log("Starting Tests for test.cgi.process.execAction close handlers tests")
    assert(options === null, "Options result for closing handler is null")
    assert(proc === 0, "Options result for closing handler is null")
    console.log("Ending Tests: with above failure test.cgi.process.execAction close handlers tests");
});
