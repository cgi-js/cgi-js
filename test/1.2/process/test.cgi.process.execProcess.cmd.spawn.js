...
let proc = obj.process.executeProcess({
    ...
    console.log("Starting Tests for test.cgi.process.executeProcess.spawn dataHandlers tests")
    ...
    console.log("Ending Tests: with above failure test.cgi.process.executeProcess.spawn dataHandlers tests")
    eventEmitter.emit('closeprocess', stdout);
}, (options, proc) => {
    console.log("Starting Tests for test.cgi.process.executeProcess.spawn close handlers tests")
    assert(options === null, "Options result for closing handler is null")
    assert(proc === 0, "Options result for closing handler is null")
    console.log("Ending Tests: with above failure test.cgi.process.executeProcess.spawn close handlers tests");
});
