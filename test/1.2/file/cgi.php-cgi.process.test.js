...
app.use("/php", response('php-cgi', { ...configuration, web_root_folder: path.join(configuration.embed.path, configuration.script.path), bin: "", config_path: lang_config, host: configuration.script.server.host, port: configuration.script.server.port, cmd_options: configuration.script.options }));
...
let server = app.listen(sport, shost, function () {
    ...
    describe('test.template.js::cgijs: Test Suite for - ', function () {
        ...
        describe('test.template.js::cgijs: [Test A] Test Suite for ', function () {
            ...
            it('[Test A]  PHP CGI server executing php file', function (done) {
                superagent.get('http://127.0.0.1:9090/php').then((r) => {
                    // console.log(r.text);
                    expect(r.text.includes("phpinfo()")).to.equal(true);
                    expect(r.text.includes("PHP Version")).to.equal(true);
                    done();
                }).catch(console.error);
            });

            it('[Test A]  PHP CGI server not executing PHP file', function (done) {
