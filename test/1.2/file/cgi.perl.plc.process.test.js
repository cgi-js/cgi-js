...
app.use("/plc", response('plc', { ...configuration, web_root_folder: path.join(configuration.embed.path, configuration.script.path), bin: "", config_path: lang_config, host: configuration.script.server.host, port: configuration.script.server.port, cmd_options: configuration.script.options }));
    ...
    describe('test.template.js::cgijs: Test Suite for - ', function () {
        ...
        describe('test.template.js::cgijs: [Test A] Test Suite for ', function () {
            ...
            it('[Test A] ', async function (done) {
                superagent.get('http://127.0.0.1:9090/plc').then((r) => {
                    // console.log(r.text);
                    expect(r.text.includes("Perl PLC Version of the page")).to.equal(true);
                    done();
                }).catch(console.error);
            });
